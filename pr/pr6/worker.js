...
for (let i = 0; i < workers.length; i++) {
    if (workers[i].addr.includes('경기도')) {
        fifthworker.push(workers[i]);
    }
}
fifthworker.sort((a, b) => a.age - b.age);
for (let i = 0; i < fifthworker.length; i++) {
    console.log(`${fifthworker[i].name} ${fifthworker[i].age} ${fifthworker[i].gender} ${fifthworker[i].number} ${fifthworker[i].addr}`);
}
