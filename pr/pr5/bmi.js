...
for (let i = 0; i < data.length; i += 3) {
    ...
    // 비만도 판정
    const Category = bmi < 18.5 ? '저체중' : (bmi < 25.0 ? '정상' : (bmi < 30.0 ? '과체중' : '비만'));
    let figBMI = fig1.substring(0, bmi) + fig0.substring(bmi, 50);
    
    console.log(`이름: ${name}`);
    console.log(`신장: ${height} cm`);
    console.log(`체중: ${weight} kg`);
    console.log(`판정: ${Category} (bmi = ${bmi})`);
    // console.log(fig0);
    console.log(`도표:`,figBMI);
    // console.log(fig1);
    console.log(`     ${fig2}`);
  }
